...
def linux_article_detail(request, id):
    ...
    article = get_object_or_404(LinuxDoc, id=id)
    ...
    # 相邻发表文章的快捷导航
    pre_article = LinuxDoc.objects.filter(id__lt=article.id).order_by('-id')
    next_article = LinuxDoc.objects.filter(id__gt=article.id).order_by('id')
...
def linux_article_create(request):
    ...
    if request.method == "POST":
        # 将提交的数据赋值到表单实例中
        article_post_form = LinuxDocForm(request.POST, request.FILES)
        # 判断提交的数据是否满足模型的要求
...
# 删除文章，此方式有 csrf 攻击风险
@login_required(login_url='/userprofile/login/')
def linux_article_delete(request, id):
    # 根据 id 获取需要删除的文章
    article = LinuxDoc.objects.get(id=id)
    # 过滤非作者的用户
...
class IncreaseLikesView(View):
    def post(self, request, *args, **kwargs):
        article = LinuxDoc.objects.get(id=kwargs.get('id'))
        article.likes += 1
        article.save()
        return HttpResponse('success')


def linux_article_list_example(request):
    """
    与下面的类视图做对比的函数
    简单的文章列表
    """
...
class ArticleListView(ContextMixin, ListView):
    ...
    # 模板
    template_name = 'linux/linux_article_list.html'

    def get_queryset(self):
        """
        查询集
        """
        queryset = LinuxDoc.objects.filter(title='Python')
        return queryset
...
class ArticleDetailView(DetailView):
    ...
    文章详情类视图
    """
    queryset = LinuxDoc.objects.all()
    context_object_name = 'article'
    template_name = 'linux/linux_article_detail.html'
...
class ArticleCreateView(CreateView):
    ...
    # 或者有选择的提交字段，比如：
    # fields = ['title']
    template_name = 'linux/linux_article_create_by_class_view.html'
